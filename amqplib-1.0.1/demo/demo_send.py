...
def main():
    ...
    msg_body = msgpack.packb({"@class": "bnow_test", "@time": "232545466", "name": "abcdefg"})
##    msg_body = ' '.join(args)
    ...
    ch.exchange_declare('amqp.direct', 'direct', auto_delete=True)

    msg = amqp.Message(msg_body, content_type='text/plain', application_headers={'foo': 7, 'bar': 'baz'})

    ch.basic_publish(msg, routing_key='bnow_request_1')
